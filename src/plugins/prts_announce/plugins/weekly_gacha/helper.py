...
async def get_latest_gacha() -> int:
    ...
    text = await bot.get_page_text(title="卡池一览", section="2")
    match = re.search(r"(?<=文件:干员轮换卡池).*?(\d+)", text)
...
async def get_announce_gacha_id(link: str):
    ...
    img_wiki_name = f'干员轮换卡池{str(latest_id+1).zfill(3)}.{pic_down_link.split(".")[-1]}'
